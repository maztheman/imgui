...
bool ImGui_ImplGlfm_KeyCallback(GLFMDisplay* display, GLFMKeyCode keycode, GLFMKeyAction action, int mods)
{
    ...
    // Block SELECT_ALL from Android IME (Ctrl+A with any action)
    if (bd->BlockSelectAll
        && keycode == GLFMKeyCodeA
        && (mods & AMETA_CTRL_ON))
    {
...
bool ImGui_ImplGlfm_TouchCallback(GLFMDisplay* display, int touch, GLFMTouchPhase phase, double x, double y)
{
    ...
    static constexpr float DRAG_SLOP  = 8.f;
    static ImVec2          s_downPos  = {};
    static bool            s_inDrag   = false;

    auto isMultilineActive = []() -> bool
    {
        ImGuiContext& g = *GImGui;
        if (g.ActiveId == 0) return false;
        ImGuiInputTextState* state = ImGui::GetInputTextState(g.ActiveId);
        return state && (state->Flags & ImGuiInputTextFlags_Multiline);
    };
    ...
    switch (phase)
    {
        ...
        {
            s_downPos = { (float)x, (float)y };
            s_inDrag  = false;

            io.AddMousePosEvent((float)x, (float)y);
            bd->LastValidMousePos = { (float)x, (float)y };

            // Clear any ghost "button held" state before registering the tap.
            // This prevents a prior unfinished gesture from poisoning this tap.
        ...
        {
            float dx = (float)x - s_downPos.x;
            float dy = (float)y - s_downPos.y;
            float d  = ImSqrt(dx * dx + dy * dy);

            if (!s_inDrag && d >= DRAG_SLOP)
                s_inDrag = true; // finger has moved enough to be a real scroll/drag

            if (isMultilineActive() && !s_inDrag)
            {
                // Still within slop inside a text box — send position update
                // but WITHOUT the button held, so ImGui repositions the cursor
                // instead of extending a selection.
                io.AddMouseButtonEvent(0, false); // <— key line
                io.AddMousePosEvent((float)x, (float)y);
                bd->LastValidMousePos = { (float)x, (float)y };
                return handled;              // skip the normal path below
            }

            // Normal move (either outside a text box, or a real intentional drag)
            io.AddMousePosEvent((float)x, (float)y);
            bd->LastValidMousePos = { (float)x, (float)y };
            break;
        }
        ...
        case GLFMTouchPhaseCancelled:
        {
            ...
            // If we never exceeded slop inside a multiline widget, any selection
            // that snuck through should be collapsed to a cursor.
            if (!s_inDrag && isMultilineActive())
            {
                ImGuiContext& g          = *GImGui;
                ImGuiInputTextState* state = ImGui::GetInputTextState(g.ActiveId);
            ...
            io.AddMousePosEvent(-FLT_MAX, -FLT_MAX); // hide cursor after lift
            s_inDrag = false;
            break;
        }
...
static bool ImGui_ImplGlfm_Init(GLFMDisplay* display, bool install_callbacks, GlfmClientApi client_api)
{
    ...
    glfmSuppressAndroidSelection(display);
...
void ImGui_ImplGlfm_SetBlockSelectAll(bool block)
{
    ImGui_ImplGlfm_Data* bd = ImGui_ImplGlfm_GetBackendData();
    if (bd) bd->BlockSelectAll = block;
}
